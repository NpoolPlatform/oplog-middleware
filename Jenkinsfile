    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep oplog-middleware | grep $branch
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
          fi
          images=`docker images | grep entropypool | grep oplog-middleware | grep none | awk '{ print $3 }'`
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/oplog-middleware:latest/oplog-middleware:$branch/g" cmd/oplog-middleware/k8s/02-oplog-middleware.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/oplog-middleware/k8s/02-oplog-middleware.yaml
          if [ "x$REPLICAS_COUNT" == "x" ];then
            REPLICAS_COUNT=2
          fi
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/oplog-middleware/k8s/02-oplog-middleware.yaml
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/oplog-middleware:latest/oplog-middleware:$tag/g" cmd/oplog-middleware/k8s/02-oplog-middleware.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/oplog-middleware/k8s/02-oplog-middleware.yaml
          if [ "x$REPLICAS_COUNT" == "x" ];then
            REPLICAS_COUNT=2
          fi
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/oplog-middleware/k8s/02-oplog-middleware.yaml
          sed -i "s/imagePullPolicy: Always/imagePullPolicy: IfNotPresent/g" cmd/oplog-middleware/k8s/02-oplog-middleware.yaml
          make deploy-to-k8s-cluster
        '''.stripIndent())
      }
    }
